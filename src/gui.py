...
def open_midifile():
    ...
    midifile_label = tk.Label(root, text=root.midifile,
             font = (("Times New Roman"), 15)).grid(row=0, column=12, columnspan=12)
...
def start():
    if root.midifile:
        status_l.text = "Running"
    else:
        print("no midifile selected")
...
Status Bar
"""
status_l = tk.Label(root, text="Not Running").grid(row=36, column=12, columnspan=12)
